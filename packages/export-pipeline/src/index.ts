...
export function powerPointStyleFidelityIssues(deck: DeckDocument): ExportPreflightIssue[] {
  ...
  for (const slide of deck.slides) for (const element of slide.scene) {
    ...
    if (element.type === "image" && (element.cornerRadiusDU ?? 0) > 0 && (element.clipShape ?? "roundRect") !== "ellipse") issues.push({ severity: "minor", lane: "export", slideId: slide.id, elementId: element.id, code: "pptx:image-corner-radius-approximate", message: "PowerPoint keeps this image as a native editable round-rectangle picture, but the exact authored Pitch image radius is approximated by the PowerPoint preset geometry." });
    if (element.type === "shape" && element.shape === "custom" && (element.fillPaint || element.effects?.length)) issues.push({ severity: "major", lane: "export", slideId: slide.id, elementId: element.id, code: "pptx:custom-vector-appearance-not-preserved", message: "Custom SVG path remains vector, but the current PowerPoint appearance pass does not inject Pitch gradient/shadow properties after SVG conversion." });
