...
export interface ComponentVariantRule {
  id: string;
  name?: string;
...
export interface ComponentVariantSet {
  ...
  rules: ComponentVariantRule[];
}

export interface ResolvedComponentVariant {
...
function matches(rule: ComponentVariantRule, selection: Record<string, string>): boolean {
  return Object.entries(rule.when).every(([axisId, value]) => selection[axisId] === value);
}
...
export function resolveComponentVariant(definition: ComponentDefinition, set: ComponentVariantSet, selection: Record<string, string> = {}): ResolvedComponentVariant {
  ...
  // Reuse the real component instantiator as final semantic validation for the composed override set.
...
export function instantiateComponentVariant(
  definition: ComponentDefinition,
  set: ComponentVariantSet,
  selection: Record<string, string>,
  transform: ComponentInstanceTransform,
  instanceId?: string,
): InstantiatedComponent & { variant: ResolvedComponentVariant } {
