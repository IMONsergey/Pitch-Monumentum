...
function relativeElement(element: SceneElement, origin: Origin): SceneElement {
  return {
    ...element,
    geometry: { ...element.geometry, x: element.geometry.x - origin.x, y: element.geometry.y - origin.y },
  } as SceneElement;
}
...
function frameContainer(element: Extract<SceneElement, { type: "frame" }>, children: string): string {
  return `<div class="pitch-el pitch-container" data-pitch-id="${esc(element.id)}" style="${base(element)};background:${backgroundPaint(element.fillPaint, element.fill)};border-radius:${element.radiusDU ?? 0}px;${element.stroke ? `border:${element.stroke.widthDU}px ${element.stroke.dash === "dash" ? "dashed" : element.stroke.dash === "dot" ? "dotted" : "solid"} ${cssColor(element.stroke.color)}` : ""};${element.clipContent ? "overflow:hidden" : "overflow:visible"}">${children}</div>`;
}
function groupContainer(element: Extract<SceneElement, { type: "group" }>, children: string): string {
  return `<div class="pitch-el pitch-container pitch-group" data-pitch-id="${esc(element.id)}" style="${base(element)};overflow:visible;background:transparent">${children}</div>`;
}
function renderLeaf(element: SceneElement, assets: Record<string, WebExportAsset>, warnings: string[], slideId: string, duPerInch: number): string {
...
function renderScene(scene: SceneElement[], assets: Record<string, WebExportAsset>, warnings: string[], slideId: string, duPerInch: number): string {
  ...
  for (const element of scene) {
    ...
    for (const childId of element.childIds) {
      if (byId.has(childId) && !parentByChild.has(childId)) parentByChild.set(childId, element.id);
    }
  }
  for (const element of scene) {
    if (element.groupId && byId.has(element.groupId) && !parentByChild.has(element.id)) parentByChild.set(element.id, element.groupId);
  }

  const childrenFor = (containerId: string): SceneElement[] => {
    const container = byId.get(containerId);
    if (!container || (container.type !== "frame" && container.type !== "group")) return [];
    const ids = new Set(container.childIds);
    for (const candidate of scene) if (candidate.groupId === containerId) ids.add(candidate.id);
    return [...ids].map((id) => byId.get(id)).filter((value): value is SceneElement => Boolean(value)).sort((a, b) => a.zIndex - b.zIndex);
  };

  const renderNode = (original: SceneElement, origin: Origin, stack: Set<string>): string => {
    if (stack.has(original.id)) {
      warnings.push(`${slideId}:${original.id} hierarchy cycle prevented during Web render`);
      return "";
    }
    const nextStack = new Set(stack); nextStack.add(original.id);
    const element = relativeElement(original, origin);
    if (original.type === "frame" || original.type === "group") {
      const children = childrenFor(original.id).map((child) => renderNode(child, { x: original.geometry.x, y: original.geometry.y }, nextStack)).join("");
      return original.type === "frame" ? frameContainer(element as Extract<SceneElement, { type: "frame" }>, children) : groupContainer(element as Extract<SceneElement, { type: "group" }>, children);
    }
    return renderLeaf(element, assets, warnings, slideId, duPerInch);
  };

  return scene.filter((element) => !parentByChild.has(element.id)).sort((a, b) => a.zIndex - b.zIndex).map((element) => renderNode(element, { x: 0, y: 0 }, new Set())).join("");
}
