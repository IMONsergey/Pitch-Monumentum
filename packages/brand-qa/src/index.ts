...
function eligible(element: SceneElement, target: ThemeBindingTarget): boolean {
  ...
  const value = uniformText(element, target as "textColor" | "fontFamily" | "fontSizePt");
  return value !== undefined && value !== "mixed";
}
...
export function runBrandQA(deck: DeckDocument, theme?: DeckTheme): BrandQAIssue[] {
  ...
  for (const slide of deck.slides) for (const element of slide.scene) {
    ...
    for (const target of TARGETS) {
      ...
      issues.push({ code: matchesTheme ? "hardcoded-brand-value" : "unknown-brand-value", severity: matchesTheme ? "minor" : "major", slideId: slide.id, elementId: element.id, target, value, message: matchesTheme ? `${target} uses a brand value literally instead of binding to its theme token` : `${target} value ${String(value)} is outside the current theme token set` });
    }
  }
  return issues;
}
