...
import { createDeliveryWorkspaceServer } from "../../workspace/src/delivery-server.js";
...
function statePath(): string { return join(app.getPath("userData"), "desktop-state-v2.json"); }
function preloadPath(): string { return join(app.getAppPath(), "dist", "apps", "desktop-next", "src", "preload.js"); }
...
async function readDesktopState(): Promise<DesktopState> {
  ...
  catch { return { schemaVersion: "0.2" }; }
}
...
async function startWorkspace(projectRoot: string): Promise<void> {
  ...
  const created = createDeliveryWorkspaceServer(projectRoot);
  workspaceServer = created.server; delivery = created.delivery;
  ...
  await writeDesktopState({ schemaVersion: "0.2", recentProjectRoot: currentProjectRoot });
}
...
function previewRoot(): string { return join(app.getPath("userData"), "Desktop Preview Project v2"); }
