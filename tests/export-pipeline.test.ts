...
function fixture(): DeckDocument {
  return {
    ...
    slides: [{ id: "s1", order: 0, title: "Business evidence", archetype: "chartInsight", semantic: { purpose: "prove trend", takeaway: "CAC declined each quarter", questionAnswered: "is efficiency improving?", narrativeRole: "evidence", claimIds: ["claim"], evidenceRefs: ["e1"], audienceRelevance: "board", density: "balanced" }, status: "ready", qaIssueIds: [], dependencyIds: ["claim"], scene: [
      { id: "title", type: "text", semanticRole: "title", geometry: { x: 140, y: 80, width: 1300, height: 120 }, zIndex: 1, origin: "deterministic", exportStrategy: "native", dependencies: [], paragraphs: [{ runs: [{ text: "CAC declined each quarter", fontSizePt: 36, bold: true, color: "#111111" }] }] },
...
test("production export gates, compiles rich native objects, round-trips and writes a manifest", async () => {
  ...
  assert.ok(manifest.editability.native >= 4);
